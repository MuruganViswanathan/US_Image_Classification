...
transform = transforms.Compose([
    transforms.Resize(224),                   # Resize shorter side to 224 pixels (Check if matches MATLAB)
    transforms.CenterCrop(224),               # Crop the center to get 224x224 images
    transforms.ToTensor()                    # Convert image to tensor
    # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normalize (Ensure this matches MATLAB)
])
...
def classify_image(image_path):
    ...
    if predictions.sum() > 1.0:  # If sum is greater than 1, apply softmax (logits case)
        probabilities = softmax(predictions)
    else:
        probabilities = predictions  # Already softmaxed in some cases
